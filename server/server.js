...
// =============================================================================
// PAYMENT ROUTES (TEMPORARY PAYMENT LINK REQUEST FLOW & RAZORPAY)
// =============================================================================

/**
 * POST /api/payment/request-link
 * Temporary flow: User requests official Razorpay payment link.
 * Records request in backend database and forwards to Google Sheet / Admin notification.
 */
app.post('/api/payment/request-link', async (req, res) => {
  try {
    ...
    console.log(`[PREMIUM REQUEST RECEIVED] ${cleanName} (${normalizedEmail}) - Phone: ${cleanPhone}`);

    // Send email alert to admin/owner asynchronously
    ...
    emailTransporter.sendMail({
      from: process.env.MARKETING_FROM_EMAIL || '"FreedomPlan" <no-reply@freedomplan.com>',
      to: adminEmail,
      ...
      html: `
        <div style="font-family:sans-serif;max-width:540px;margin:auto;padding:28px;background:#ffffff;border-radius:14px;border:1px solid #E5E7EB;">
          ...
          <p style="color:#4B5563;font-size:14px;line-height:1.5;">A customer has requested the official Razorpay payment link for <strong>FreedomPlan Premium</strong>.</p>
          <div style="background:#F9FAFB;border-radius:10px;padding:16px 20px;margin:20px 0;border:1px solid #E5E7EB;font-size:14px;line-height:1.8;">
            ...
            <div><strong>Status:</strong> Payment Link Requested</div>
            <div><strong>Date:</strong> ${new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' })}</div>
          </div>
          <p style="color:#6B7280;font-size:12px;">Next Step: Send the official Razorpay payment link to this customer. Once verified, approve their email in FreedomPlan.</p>
        </div>
      `,
    }).catch(mailErr => {
      console.warn('[ADMIN NOTIFICATION WARNING]', mailErr.message);
    });

    // Secure backend sync to Google Sheets (non-blocking)
    syncToGoogleSheet({
      'Name': cleanName,
      'Email': normalizedEmail,
      'Phone': cleanPhone,
      'Plan': plan,
      'Amount': amount,
      'Payment Status': 'Payment Link Requested',
    ...
    res.json({
      success: true,
      message: 'Payment link requested successfully',
      data: requestData,
    });
  } catch (err) {
