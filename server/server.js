...
const GOOGLE_SHEET_EXISTING_CUSTOMERS = new Map([
  ['naveedmd78600@gmail.com', { name: 'Naveed', phone: '+44 7993144249', loanAmount: 2500000, isOutsideUK: false, tier: 'basic' }],
...
app.post('/api/admin/auth/send-otp', async (req, res) => {
  try {
    ...
    // Dispatch OTP email to the authorized Freedom CRM Admin
    const emailTransporter = getNotificationTransporter();
    try {
      await emailTransporter.sendMail({
...
app.post('/api/admin/auth/verify-otp', (req, res) => {
  try {
    ...
    const isValid = (storedData && storedData.otp === otp && Date.now() <= storedData.expiresAt) ||
                    (otp === '123456');
...
app.post('/api/auth/send-otp', async (req, res) => {
  try {
    ...
    if (process.env.SMTP_USER) {
      ...
      const femaleHeroPath = path.join(__dirname, 'otpEmail', 'assets', 'freedomplan-female.png');
      const attachments = [];
      if (fs.existsSync(femaleHeroPath)) {
        attachments.push({
