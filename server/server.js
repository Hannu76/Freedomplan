...
app.post('/api/admin/send-bulk-promotional-links', async (req, res) => {
  try {
    const { getValidatedCampaignAudience } = require('./marketing/campaignAudienceValidator');
    const { renderPromotionalTemplate } = require('./marketing/emailRenderer');
    const audienceReport = getValidatedCampaignAudience({ additionalRecords: cachedSheetCustomers });
    const validAudience = audienceReport.validAudience;

    const mailer = getNotificationTransporter();
    const flyerPath = path.join(__dirname, 'marketing', 'assets', 'freedomplan-promo-flyer.jpg');
    const hasFlyer = fs.existsSync(flyerPath);

    let dispatchedCount = 0;
    for (const customer of validAudience) {
      const cleanEmail = customer.email;
      const cleanName = customer.name || cleanEmail.split('@')[0];
      ...
      if (mailer) {
        ...
        mailer.sendMail({
          ...
          console.log(`[PROMO EMAIL DELIVERED] Dispatched Image 2 template to ${cleanEmail}`);
        }).catch(err => {
          console.warn(`[PROMO EMAIL WARNING] ${cleanEmail}:`, err.message);
        });
      }
      dispatchedCount++;
    }
