 ...
 * Business Rule:
 * Registered/Customer + Marketing Consent = true + Not Unsubscribed + Valid Email
 * (Purchase status is NOT required)
 */
function getEligibleAudience() {
  const subscribers = readJSON(SUBSCRIBERS_FILE, []);
  const unsubscribes = new Set(readJSON(UNSUBSCRIBES_FILE, []).map(u => cleanEmail(u.email)));

  const audienceMap = new Map();

  // 1. Process explicit marketing subscribers
  for (const sub of subscribers) {
    const email = cleanEmail(sub.email);
    if (!email || !email.includes('@')) continue;
    if (unsubscribes.has(email)) continue;
    if (sub.status === 'active' || sub.consent === true) {
      audienceMap.set(email, {
        email,
        name: sub.name || email.split('@')[0],
        source: sub.source || 'subscriber',
      });
    }
  }

  // 2. Read existing customer identities (e.g. paidEmails.json) without altering them
  const paidEmailsFile = path.join(__dirname, '..', 'paidEmails.json');
  if (fs.existsSync(paidEmailsFile)) {
    try {
      const paidData = JSON.parse(fs.readFileSync(paidEmailsFile, 'utf8'));
      if (Array.isArray(paidData)) {
        for (const rawEmail of paidData) {
      ...
      console.warn('[MARKETING STORE] Note reading paidEmails.json:', err.message);
    }
  }
