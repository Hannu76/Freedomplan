  ...
  try {
    ...
    if (mailer) {
      try {
        ...
        console.log(`[PAYMENT LINK EMAIL DELIVERED] Dispatched to ${cleanEmail}`);
      } catch (mailErr) {
...
app.post('/api/admin/send-bulk-promotional-links', async (req, res) => {
  try {
    ...
    for (const cust of validAudience) {
      const paymentUrl = `${process.env.APP_URL || 'https://freedomplan.vercel.app'}/?payment=true&email=${encodeURIComponent(cust.email)}&promo=friday_special`;
      if (mailer) {
        mailer.sendMail({
          ...
          html: `
            <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 540px; margin: 0 auto; padding: 24px; border: 1px solid #e2e8f0; border-radius: 16px; background: #ffffff;">
              ...
              <p style="color: #1e293b; font-size: 14px; line-height: 1.6;">Hello <strong>${cust.name || cust.email.split('@')[0]}</strong>,</p>
          ...
          `,
        }).catch(e => console.warn('[BULK PROMO MAIL ERR]', cust.email, e.message));
      }
      count++;
    }
