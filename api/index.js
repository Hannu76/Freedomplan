...
app.post('/api/admin/send-customer-payment-link', async (req, res) => {
  try {
    ...
    const paymentUrl = `${process.env.APP_URL || 'https://hannu76.github.io/Freedomplan'}/?payment=true&email=${encodeURIComponent(cleanEmail)}&plan=${encodeURIComponent(plan)}`;
    ...
    if (process.env.SMTP_USER && process.env.SMTP_PASS) {
      try {
        ...
        await transporter.sendMail({
          from: `"FreedomPlan Billing" <${process.env.SMTP_USER}>`,
          to: cleanEmail,
          subject: 'Complete Your FreedomPlan Premium Upgrade 🚀',
          html: `<p>Hello ${name || cleanEmail.split('@')[0]},</p><p>Your payment link for ${plan} is ready: <a href="${paymentUrl}">Activate Premium</a></p>`,
        });
      } catch (mailErr) {
    ...
    res.json({
      success: true,
      message: `Payment link prepared and sent to ${cleanEmail}`,
      paymentUrl,
    });
  } catch (err) {
...
app.post('/api/admin/send-bulk-promotional-links', async (req, res) => {
  res.json({ success: true, message: 'Bulk promotional payment links prepared successfully.' });
});
