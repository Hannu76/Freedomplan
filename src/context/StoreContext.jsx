...
export function StoreProvider({ children }) {
  ...
  const isPremiumUser = Boolean(currentUser?.premium || currentUser?.tier === 'pro')
  ...
  // Reactive synchronization: Keep primary loan EMI account in sync with coApplicantContribution as single source of truth
  React.useEffect(() => {
    const fee = hasCoApplicant ? Number(coApplicantContribution) || 0 : 0
    setAccounts((prev) => {
      if (!Array.isArray(prev)) return prev
      let changed = false
      const next = prev.map((a) => {
        if (a.id === 'india-loan-emi' && a.targetAmount !== fee) {
          changed = true
          return { ...a, targetAmount: fee, status: fee > 0 ? 'Active' : 'Paused' }
        }
        return a
      })
      return changed ? next : prev
    })
  }, [coApplicantContribution, hasCoApplicant, setAccounts])
  ...
  function updateAccount(id, patch) {
    if (id === 'india-loan-emi' && patch.targetAmount !== undefined) {
      const newFee = Number(patch.targetAmount) || 0
      setCoApplicantContribution(newFee)
    }
  ...
  const derived = useMemo(() => {
    ...
    const effectiveMonthlyEMI = (hasCoApplicant ? Number(coApplicantContribution) : 0) || LOAN.monthlyEMIINR || 5000
    const fixedEMIAllYears = effectiveMonthlyEMI * termMonths
    ...
    return {
      ...
      creditAdvanceFee,
      netAmountReceived,
      monthlyEMIINR: effectiveMonthlyEMI,
    }
  ...
  const value = {
    ...
    setHasCoApplicant,
    isPremiumUser,
    monthlyEMIINR: (hasCoApplicant ? Number(coApplicantContribution) : 0) || LOAN.monthlyEMIINR || 5000,
    entries,
    getEntry,
    updateEntry,
    generalNotes,
