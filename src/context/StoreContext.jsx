...
export function StoreProvider({ children }) {
  ...
  // Target Alert Modal state & update completion validation workflow
  const [targetAlertData, setTargetAlertData] = React.useState(null)
  const [isTargetAlertOpen, setIsTargetAlertOpen] = React.useState(false)

  const completeValueUpdate = React.useCallback((options = {}) => {
    // 1. Play completion sound ONCE after system finishes calculating and updating all numbers
    playDoneSound()

    // 2. Perform monthly target validation
    ...
    // 3. If target not reached (remaining difference > 0), show animated popup automatically
  ...
  const closeTargetAlert = React.useCallback(() => {
    setIsTargetAlertOpen(false)
  }, [])
