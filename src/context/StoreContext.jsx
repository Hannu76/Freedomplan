...
export function StoreProvider({ children }) {
  const [darkMode, setDarkMode] = useLocalStorage('freedomPlan.darkMode', true)
  const [soundEnabled, setSoundEnabled] = useLocalStorage('freedomPlan.soundEnabled', true)

  // GBP -> INR rate, user-editable
  const [rawRate, setRate] = useLocalStorage('freedomPlan.rate', CURRENCY.defaultRate)
  const rate = Number(rawRate) && Number(rawRate) > 0 ? Number(rawRate) : CURRENCY.defaultRate

  React.useEffect(() => {
    fetch('https://api.frankfurter.app/latest?from=GBP&to=INR')
      .then(res => res.json())
      .then(data => {
  ...
  // Initial Loan Principal (Editable)
  const [rawBasicLoan, setBasicLoan] = useLocalStorage('freedomPlan.basicLoan', 2700000)
  const basicLoan = Number(rawBasicLoan) > 0 ? Number(rawBasicLoan) : 2700000

  // Per-month savings entries, keyed by "YYYY-MM":
  const [rawEntries, setEntries] = useLocalStorage('freedomPlan.entries', {})
  ...
  const [generalNotes, setGeneralNotes] = useLocalStorage('freedomPlan.generalNotes', '')

  // Multi-Account AMS management
  const [rawAccounts, setAccounts] = useLocalStorage('freedomPlan.accounts', DEFAULT_ACCOUNTS)
  const accounts = Array.isArray(rawAccounts) ? rawAccounts : DEFAULT_ACCOUNTS

  // Amortization Simulator Sandbox state
  const [rawSimulator, setSimulator] = useLocalStorage('freedomPlan.simulator', {
  ...
  const [rawCustomPlan, setCustomPlan] = useLocalStorage('freedomPlan.customPlan.v2', {
  ...
  const customPlan = useMemo(() => {
    // Dynamically calculate the monthly savings target based on the loan amount
    const termMonths = 36
    const fixedEMIAllYears = LOAN.monthlyEMIINR * termMonths
    const targetYearlyLumpSumINR = Math.max(0, Math.floor((basicLoan - fixedEMIAllYears) / 3))
  ...
  }, [rawCustomPlan, basicLoan, rate])

  function updateCustomPlan(patch) {
    setCustomPlan((prev) => ({ ...prev, ...patch }))
  }

  // Automatically recalibrate and sync the monthly prepayment target whenever the loan balance or exchange rate changes
  React.useEffect(() => {
    const termMonths = 36
    const fixedEMIAllYears = LOAN.monthlyEMIINR * termMonths
    const targetYearlyLumpSumINR = Math.max(0, Math.floor((basicLoan - fixedEMIAllYears) / 3))
  ...
  const [rawTimelineConfig, setTimelineConfig] = useLocalStorage('freedomPlan.timelineConfig', {
  ...
  const [isBasicUnlocked, setIsBasicUnlocked] = useLocalStorage('freedomPlan.isBasicUnlockedV2', false)
  const [isProUnlocked, setIsProUnlocked] = useLocalStorage('freedomPlan.isProUnlockedV2', false)
  const [proLeadData, setProLeadData] = useLocalStorage('freedomPlan.proLeadData', null)

  const [strategyChecks, setStrategyChecks] = useLocalStorage('freedomPlan.strategyChecks', [true, false, false, false])
  ...
  const derived = useMemo(() => {
    ...
    // We assume a 3-year term (36 months).
    const termMonths = 36
    const fixedEMIAllYears = LOAN.monthlyEMIINR * termMonths
    // How much of the basicLoan MUST be cleared via the 3 December lump sums (rounded down to nearest whole INR):
    const targetYearlyLumpSumINR = Math.max(0, Math.floor((basicLoan - fixedEMIAllYears) / 3))
    ...
    // If currentMonthIdx is -1, fallback to 1. Else at least 1 month has elapsed.
  ...
  }, [timeline, entries, currentKey, accounts, rate, basicLoan])
