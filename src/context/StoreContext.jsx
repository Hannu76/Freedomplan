...
export function StoreProvider({ children }) {
  ...
  const profilePrefix = isLoggedIn && currentUser?.email ? `freedomPlan.${currentUser.email}` : 'freedomPlan.guest'
  ...
  const [rawBasicLoan, setBasicLoan] = useLocalStorage(`${profilePrefix}.basicLoan`, 2700000)
  const basicLoan = Number(rawBasicLoan) > 0 ? Number(rawBasicLoan) : 2700000

  const [interestRate, setInterestRate] = useLocalStorage(`${profilePrefix}.interestRate`, 11)
  ...
  const customPlan = useMemo(() => {
    ...
    // Revert to original strategy: Base it directly on basicLoan instead of P_grad
    const targetYearlyLumpSumINR = Math.max(0, Math.floor((basicLoan - fixedEMIAllYears) / planYears))
  ...
  const derived = useMemo(() => {
    ...
    const R_monthly = interestRate / 12
    const I_month = basicLoan * (R_monthly / 100)
    const coApplicant = hasCoApplicant ? coApplicantContribution : 0
    const studentShare = Math.max(0, I_month - coApplicant)
    const I_unpaidTotal = studentShare * moratoriumMonths
    ...
    const targetYearlyLumpSumINR = Math.max(0, Math.floor((P_grad - fixedEMIAllYears) / 3))
