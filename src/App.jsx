...
function getAvatarImage(user) {
  if (!user) return '/avatar-male.png';
  ...
  return isFemale ? '/avatar-female.png' : '/avatar-male.png';
}
...
function AuthPopoverContent({ onSuccess, onCreateAccount }) {
  ...
  const handleVerifyOtp = async (e) => {
    ...
    const completeLogin = () => {
      ...
      let loggedInUser = users.find(u => u.email?.toLowerCase().trim() === cleanEmail);
      ...
      if (!loggedInUser) {
        ...
        updatedUsers = users.map(u =>
          u.email?.toLowerCase().trim() === cleanEmail ? { ...u, lastLogin: timestamp } : u
        );
      }
    ...
    if (isLocalhost) {
      setTimeout(() => {
        if (otp !== '123456') setError('Invalid OTP. Use 123456.');
        else completeLogin();
        setIsLoading(false);
      }, 500);
      return;
  ...
  return (
    <div className="p-5">
      <h3 className="text-sm font-extrabold text-[#161C2D] dark:text-white mb-4">Welcome back</h3>
      {step === 'email' ? (
        <form onSubmit={handleSendOtp} className="flex flex-col gap-3">
          ...
          <button
            disabled={isLoading}
            type="submit"
            className="w-full mt-1 bg-[#001C44] dark:bg-white hover:opacity-90 text-white dark:text-[#161C2D] text-xs font-extrabold tracking-wide uppercase py-3 rounded-xl transition-all active:scale-95 disabled:opacity-50"
          >
        ...
        <form onSubmit={handleVerifyOtp} className="flex flex-col gap-3">
          ...
          <button
            disabled={isLoading}
            type="submit"
            className="w-full mt-1 bg-blue-600 hover:bg-blue-700 text-white text-xs font-extrabold tracking-wide uppercase py-3 rounded-xl transition-all active:scale-95 disabled:opacity-50"
          >
            {isLoading ? 'Wait...' : 'Verify Login'}
          </button>
          <button
            type="button"
...
function AccountDropdown({ user, isPremium, onLogout, onLogin }) {
  ...
  return (
    <div className="relative inline-block text-left shrink-0" ref={dropdownRef}>
      ...
      >
        {user ? (
          <img src={getAvatarImage(user)} alt="User Avatar" className="w-full h-full object-cover rounded-full" />
        ) : (
          "Log in"
        )}
      </button>
...
function AppShell({ isAppReady = true }) {
  const { darkMode, setDarkMode, soundEnabled, setSoundEnabled, rate, setRate, isBasicUnlocked, setIsBasicUnlocked, isProUnlocked, setIsProUnlocked, setProLeadData, isLoggedIn, currentUser, setCurrentUser, setSessionLoginTime, reportModal, openReportModal, closeReportModal } = useStore()
  ...
  return (
    <>
              ...
              <div className="order-2 lg:order-3 flex items-center gap-2 shrink-0">
                ...
                <div className="relative group shrink-0 hidden sm:block">
                  ...
                  <button
                    ...
                    className="relative z-10 py-1.5 px-4 rounded-full font-extrabold uppercase tracking-wide text-[10px] text-[#052E16] transition-all bg-[#98CD3F] hover:opacity-90 shadow-xl border border-[#7DB425]/50 flex items-center justify-center gap-1.5 overflow-hidden whitespace-nowrap active:scale-95"
                    style={{
      ...
      <PaymentModal
        ...
        onSuccess={() => {
          setIsPaymentModalOpen(false)
          // Navigate to the pending protected tab (or stay on current tab)
          if (pendingProtectedTab && pendingProtectedTab !== 'dashboard') {
            handleTabClick(pendingProtectedTab)
          }
          setPendingProtectedTab(null)
        }}
      />
