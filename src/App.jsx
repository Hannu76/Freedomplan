...
function AuthPopoverContent({ onSuccess, onCreateAccount }) {
  ...
  const handleVerifyOtp = async (e) => {
    ...
    const completeLogin = () => {
      ...
      let loggedInUser = users.find(u => u.email?.trim().toLowerCase() === cleanEmail);
      ...
      if (!loggedInUser) {
        ...
        updatedUsers = users.map(u =>
          u.email?.trim().toLowerCase() === cleanEmail ? { ...u, lastLogin: timestamp } : u
        );
      }
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-12 md:gap-8">
                ...
                <div>
                  ...
                  <ul className="space-y-3 text-sm text-slate-800 font-semibold">
                    ...
                    {isLocalhost && (
                      <li><button onClick={() => setIsMarketingModalOpen(true)} className="hover:text-blue-600 transition-colors text-left flex items-center gap-1.5 text-xs text-blue-600 font-semibold">Dev Marketing Console</button></li>
                    )}
