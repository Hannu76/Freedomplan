...
function AppShell({ isAppReady = true }) {
  const { darkMode, setDarkMode, soundEnabled, setSoundEnabled, rate, setRate, isBasicUnlocked, setIsBasicUnlocked, isProUnlocked, setIsProUnlocked, setProLeadData, isLoggedIn, reportModal, openReportModal, closeReportModal } = useStore()
  ...
  useEffect(() => {
    ...
    if (forcePage && ['404', 'maintenance'].includes(forcePage)) {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (!headerBar) return;
      // Trigger exactly as the "Your Financial Roadmap" section enters view (approx 0.85 to 0.9 of 100vh)
  ...
  const isFullPage = activeTab === '404' || activeTab === 'maintenance';
  ...
  return (
    <>
      ...
      <div className={`dark-invert min-h-screen flex flex-col text-[#161C2D] transition-colors duration-350 selection:bg-[#B6F36A] selection:text-[#161C2D] ${!darkMode ? 'bg-[#FFFFFF]' : 'bg-transparent'}`}>
        {!isFullPage && (
          <header id="main_app_header" className={`${isLandingPage ? 'fixed left-0 right-0' : 'sticky'} transition-all duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] top-4 z-50 max-w-[1220px] w-[92%] mx-auto ${!isLandingPage ? 'mb-8' : ''}`}>
            <div className="flex flex-col lg:flex-row items-center justify-between p-2 pl-6 pr-2 rounded-[32px] bg-white backdrop-blur-xl shadow-sm border border-neutral-200/80 transition-all gap-4 lg:gap-0">

              {/* Brand Logo & Title */}
              <div className="flex items-center gap-2 cursor-pointer mr-6" onClick={() => setActiveTab('dashboard')}>
                <span className="font-sans font-bold text-[19px] tracking-tight text-[#161C2D]">
                  Freedom<span className="text-[#93E33C]">plan</span>
                </span>
                <span className="text-[9.5px] uppercase font-bold px-2.5 py-0.5 rounded-full bg-transparent text-[#64748B] tracking-wider border border-[#E2E8F0]">
                  Basic
                </span>
              </div>

              {/* Centered Pill Navigation Bar */}
              <div className="hidden lg:flex justify-center py-1 text-center">
                <nav className="inline-flex items-center bg-transparent rounded-full">
                  {TABS.map((tab) => {
                    ...
                    return (
                      <button
                        ...
                        className={`relative rounded-full px-5 py-2 text-[13px] tracking-wide transition-all duration-300 ${isActive ? 'bg-[#0F172A] text-white font-semibold shadow-sm mx-1' : 'text-[#64748B] hover:text-[#0F172A] font-medium mx-1'}`}
                      >
                        {tab.label}
                      </button>
                    )
              ...
              <div className="hidden lg:flex items-center gap-1.5 shrink-0">
                {/* Clean Rate Editor */}
                <div
                  className="flex items-center bg-white rounded-full pl-3 pr-3 py-1.5 border border-[#EEF2F7] shadow-sm transition-colors hover:border-neutral-200"
                  title="Live Exchange Rate"
                >
                  <span className="text-[#4A7BFF] mr-2 flex items-center justify-center">
                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="relative mt-[1px] opacity-80">
                      <polyline points="22 7 13.5 15.5 8.5 10.5 2 17"></polyline>
                      <polyline points="16 7 22 7 22 13"></polyline>
                      <circle cx="22" cy="7" r="4" fill="#ef4444" stroke="none" className="animate-pulse" />
                      <circle cx="22" cy="7" r="2.5" fill="#ef4444" stroke="#ffffff" strokeWidth="1" />
                    </svg>
                  </span>
                  <span className="text-[#64748B] text-[13px] font-semibold mr-0.5">₹</span>
                  <input
                    type="text"
                    inputMode="decimal"
                    value={tempRate}
                    ...
                    className="w-[50px] text-right bg-transparent text-[#0F172A] figure text-[13px] font-bold focus:outline-none focus:text-[#4A7BFF] transition-colors -mb-[1px]"
                  />
                  <span className="text-[#64748B] text-[13px] font-semibold ml-1">/ £</span>
                </div>

                {/* Enquiry Button */}
                <button
                  onClick={() => setIsProModalOpen(true)}
                  className="bg-gradient-to-b from-[#2A2A2D] to-[#0A0A0A] text-white hover:from-[#353538] hover:to-[#0F0F0F] transition-all px-[20px] py-[8px] rounded-full text-[11px] font-black tracking-widest uppercase border border-[#000] shadow-[inset_0px_1px_rgba(255,255,255,0.15),_0_4px_12px_rgba(0,0,0,0.15)] active:scale-95"
                >
                  Enquiry
                </button>
              </div>
              ...
              <div className="flex lg:hidden items-center justify-center gap-2 w-full pb-2">
                <button
                  onClick={() => {
                    setTempRate(rate.toFixed(2))
                    setIsEditingRate(true)
                  }}
                  className="px-4 py-2 rounded-full bg-white border border-[#EEF2F7] text-xs font-bold text-[#111111] shadow-sm flex items-center justify-center"
                >
                  <span className="text-[#4A7BFF] mr-2">
                ...
                <button
                  onClick={() => setIsProModalOpen(true)}
                  className="bg-[#111111] text-white px-5 py-2 rounded-full text-xs font-black tracking-widest uppercase border border-[#111111]"
                >
                  Enquiry
                </button>
              </div>
        ...
        <main
          className={
            isFullPage ? "flex-1 w-full mx-auto overflow-hidden flex flex-col justify-center" :
              isLandingPage ? "w-full min-h-screen" :
                "flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 py-4 pb-0 overflow-hidden"
          }
        ...
        >
          <SlideTransition activeTab={activeTab}>
            ...
            {activeTab === 'charts' && (
              <BlurGate isLocked={!isLocalhost && !isLoggedIn} onUnlock={() => { setPendingProtectedTab(activeTab); setIsProModalOpen(true); }}>
                <AnalyticsDashboard
                  onRequirePro={() => setIsProModalOpen(true)}
