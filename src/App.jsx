  ...
  return (
    ...
    >
      ...
      <button
        ...
        onClick={() => setIsOpen(!isOpen)}
        className="pointer-events-auto w-[52px] h-[52px] min-w-[52px] min-h-[52px] rounded-full flex items-center justify-center font-bold text-sm tracking-wide active:scale-95 transition-all overflow-hidden bg-white/60 dark:bg-black/40 backdrop-blur-md shadow-[0_8px_32px_rgba(0,0,0,0.15)] border border-white/80 dark:border-white/15 text-[#0F172A] dark:text-white hover:scale-105"
        title={user ? user.name : "Log in"}
      >
        {user ? (
          <img src={getAvatarImage(user)} alt="User Avatar" className="w-[75%] h-[75%] object-contain rounded-full" />
        ) : (
          <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" className="text-slate-800 dark:text-slate-200">
...
function getNavTier(tabId) {
  if (tabId === 'dashboard' || tabId === 'charts') return 'silver';
  if (tabId === 'tracker' || tabId === 'converter') return 'gold';
  if (tabId === 'accounts' || tabId === 'schedule' || tabId === 'loan') return 'platinum';
  return 'silver';
}

function SlideTransition({ activeTab, children }) {
  return (
    <div className="w-full flex-1 overflow-hidden min-h-[calc(100vh-80px)]">
...
function AppShell({ isAppReady = true }) {
  const { darkMode, setDarkMode, soundEnabled, setSoundEnabled, rate, setRate, isBasicUnlocked, setIsBasicUnlocked, isProUnlocked, setIsProUnlocked, isPremiumUser, setProLeadData, isLoggedIn, setIsLoggedIn, currentUser, setCurrentUser, setSessionLoginTime, reportModal, openReportModal, closeReportModal } = useStore()
  const [activeTab, setActiveTab] = useState('dashboard')
  const navScrollContainerRef = useRef(null)

  // Mobile only: minimal horizontal auto-slide to reveal active tab ONLY if cut off (no centering)
  useEffect(() => {
    ...
    const padding = 12; // Safety margin for comfortable edge spacing

    // If active tab is cut off on the right edge
    if (btnRect.right > containerRect.right) {
      const diff = btnRect.right - containerRect.right + padding;
      container.scrollTo({
        left: container.scrollLeft + diff,
        behavior: 'smooth'
      });
    }
    // If active tab is cut off on the left edge
    else if (btnRect.left < containerRect.left) {
      const diff = containerRect.left - btnRect.left + padding;
      container.scrollTo({
        left: Math.max(0, container.scrollLeft - diff),
        behavior: 'smooth'
      });
    }
  }, [activeTab])
  ...
  useEffect(() => {
    // Layout scroll listener: hide header over hero on Landing page, reveal when scrolled past hero
    const handleScroll = () => {
  ...
  // Explicit trigger check for ?admin=true or ?login=true
  useEffect(() => {
  ...
  const handleTriggerWhatsApp = React.useCallback(() => {
    window.open('https://chat.whatsapp.com/C0LgZSttstQK14lgFHSdWA', '_blank', 'noopener,noreferrer');
  }, []);
  ...
  // Guest popup timer: ONLY triggers when user is on tools/tabs other than hero landing page
  useEffect(() => {
    if (isLoggedIn) return;
    if (activeTab === 'dashboard') return; // Suppress on hero landing page
  ...
  return (
    <>
      ...
      <div className={`dark-invert min-h-screen flex flex-col text-[#161C2D] transition-colors duration-350 selection:bg-[#00cc44] selection:text-[#161C2D] ${!darkMode ? 'bg-[#FFFFFF]' : 'bg-transparent'}`}>
        {!isFullPage && (
          <header id="main_app_header" className={`fixed top-0 left-0 right-0 z-[100] w-full bg-white/80 dark:bg-[#0D0F14]/80 backdrop-blur-xl border-b border-slate-200/60 dark:border-slate-800/60 shadow-sm transition-[opacity,transform] duration-300 ${activeTab === 'dashboard' ? 'opacity-0 pointer-events-none -translate-y-5' : 'opacity-100 translate-y-0'}`}>
            <div className="max-w-[1280px] w-full mx-auto px-3 sm:px-6 py-2 flex flex-wrap lg:grid lg:grid-cols-[1fr_auto_1fr] items-center justify-between lg:justify-normal gap-2 lg:gap-4">

              {/* Left Side: Brand Logo & Title */}
              <div
                className="flex items-center gap-2.5 cursor-pointer shrink-0 lg:justify-self-start"
                onClick={() => setActiveTab('dashboard')}
                title={isPremiumUser ? 'Freedom Plan Premium' : 'Freedom Plan Basic'}
                aria-label={isPremiumUser ? 'Freedom Plan Premium' : 'Freedom Plan Basic'}
              >
                <img
                  src="/images/freedom-plan-logo.png"
                  alt="FreedomPlan"
                  className="h-9 sm:h-10 md:h-11 lg:h-12 xl:h-[52px] w-auto max-w-[140px] sm:max-w-[170px] md:max-w-[200px] xl:max-w-[220px] object-contain drop-shadow-xs transition-transform duration-200 hover:scale-105"
                />
                <span className={`text-[9px] uppercase font-bold px-2.5 py-0.5 rounded-full tracking-wider border shadow-xs flex items-center gap-1.5 transition-colors ${
                  isPremiumUser
                    ? 'bg-amber-50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-300 border-amber-300 dark:border-amber-800'
                    : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-700'
                }`}>
                  <span className={`w-1.5 h-1.5 rounded-full ${isPremiumUser ? 'bg-amber-500 shadow-[0_0_6px_rgba(245,158,11,0.6)]' : 'bg-slate-400'}`} />
                  {isPremiumUser ? 'Premium' : 'Basic'}
                </span>
              </div>

              {/* Center: Pill Navigation Bar (Mathematically Centered on Desktop, Full Width Scroll on Mobile) */}
              <div
                ref={navScrollContainerRef}
                className="order-3 lg:order-none w-full lg:w-auto flex justify-start lg:justify-center overflow-x-auto scrollbar-none py-0.5 text-center lg:justify-self-center"
              >
                <nav className="premium-nav nav-tier-track grid grid-cols-6 p-1 rounded-full shrink-0 whitespace-nowrap min-w-[460px] sm:min-w-[500px]">
                  {/* Layer 2: Animated Glass Glider (Single Tab Highlight) */}
                  <div
                    className="glass-glider"
                    data-tier={getNavTier(activeTab)}
                    style={{
                      transform: `translateX(${Math.max(0, TABS.findIndex(t => t.id === activeTab)) * 100}%)`
                    }}
                  />

                  {/* Layer 3: Interactive Navigation Tab Items */}
                  {TABS.map((tab) => {
                    ...
                    return (
                      <button
                        ...
                        className={`nav-tab relative z-10 w-full h-full flex items-center justify-center rounded-full px-2 sm:px-3 py-1 sm:py-1.5 text-xs text-center font-semibold tracking-wide transition-colors duration-200 ${
                          isActive
              ...
              <div className="order-2 lg:order-none flex items-center gap-2 shrink-0 lg:justify-self-end">
                {/* Live Rate Editor */}
                <div
                ...
                <div className="relative group shrink-0 hidden sm:block">
                  <div className="absolute -bottom-1.5 left-1/2 -translate-x-1/2 w-4/5 h-3 bg-gradient-to-r from-emerald-600 via-[#00cc44] to-emerald-500 blur-md rounded-full opacity-80 group-hover:opacity-100 group-hover:blur-lg transition-all duration-300 pointer-events-none" />
                  <a
                ...
                {/* Desktop Account Control: Shown exclusively when logged in */}
                {isLoggedIn && (
        ...
        <main
          className={
            isFullPage ? "flex-1 w-full mx-auto flex flex-col justify-center overflow-x-hidden" :
              isLandingPage ? "w-full min-h-screen overflow-x-hidden" :
                "flex-1 max-w-7xl w-full mx-auto px-3 sm:px-6 pt-32 sm:pt-36 lg:pt-24 pb-12 sm:pb-16 overflow-x-hidden"
          }
        ...
        >
          <SlideTransition activeTab={activeTab}>
            ...
            {(activeTab === 'schedule' || activeTab === 'loan') && (
              <BlurGate isLocked={!isProUnlocked} title="Unlock Premium Access" message="Unlock Premium Access to access this exclusive feature." onUnlock={() => { setPendingProtectedTab(activeTab); !isLoggedIn ? setIsProModalOpen(true) : setIsPaymentModalOpen(true); }}>
                <LoanTracker />
              </BlurGate>
            )}
            ...
            {activeTab === 'about' && (
              <AboutSection
                onOpenLogin={() => setIsProModalOpen(true)}
                onOpenRegister={() => setIsProModalOpen(true)}
            ...
            {!['dashboard', 'charts', 'accounts', 'tracker', 'schedule', 'converter', 'loan', 'about', '404', 'maintenance'].includes(activeTab) && (
              <NotFoundPage onGoHome={() => setActiveTab('dashboard')} />
            )}
          </SlideTransition>
        </main>
            ...
            <div className="max-w-7xl mx-auto px-6 py-12 md:py-16 relative z-10">
              ...
              <div className="border-t border-black/10 mt-12 pt-8 flex flex-col md:flex-row items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-4 text-xs font-bold text-slate-800">
                  <a href="https://www.freedomplan.guru" target="_blank" rel="noopener noreferrer" className="hover:text-black transition-colors">freedomplan.guru</a>
