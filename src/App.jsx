...
function AppShell({ isAppReady = true }) {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      if (!headerBar) return;
      // On mobile / small screens, always keep header visible and stable
  ...
  return (
    <>
      ...
      <div className={`dark-invert min-h-screen flex flex-col text-[#161C2D] transition-colors duration-350 selection:bg-[#B6F36A] selection:text-[#161C2D] ${!darkMode ? 'bg-[#FFFFFF]' : 'bg-transparent'}`}>
        {!isFullPage && (
          <header id="main_app_header" className={`fixed top-0 left-0 right-0 z-[100] w-full bg-white/80 dark:bg-[#0D0F14]/80 backdrop-blur-xl border-b border-slate-200/60 dark:border-slate-800/60 shadow-sm max-lg:!opacity-100 max-lg:!translate-y-0 max-lg:!pointer-events-auto transition-[opacity,transform] duration-300`}>
