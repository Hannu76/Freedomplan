      ...
      <div className={`dark-invert min-h-screen flex flex-col text-[#161C2D] transition-colors duration-350 selection:bg-[#B6F36A] selection:text-[#161C2D] ${!darkMode ? 'bg-[#FFFFFF]' : 'bg-transparent'}`}>
        ...
        >
          <SlideTransition activeTab={activeTab}>
            ...
            {activeTab === 'accounts' && (
              <BlurGate isLocked={!isProUnlocked} title="Unlock Pro Access" message="Unlock Pro Access to access this premium feature." onUnlock={() => { setPendingProtectedTab(activeTab); setIsPaymentModalOpen(true); }}>
                <AccountsManager />
              </BlurGate>
            )}
            {activeTab === 'tracker' && (
            ...
            {activeTab === 'payment-success' && (
              <PaymentSuccess onComplete={() => setActiveTab('dashboard')} />
            )}
            {!['dashboard', 'charts', 'accounts', 'tracker', 'converter', 'loan', '404', 'maintenance', 'payment-success'].includes(activeTab) && (
              <NotFoundPage onGoHome={() => setActiveTab('dashboard')} />
            )}
          </SlideTransition>
        </main>
