...
export default function AdminSubscriptionModal({ isOpen, onClose }) {
  ...
  const fetchCampaignAudience = async () => {
    try {
      const res = await fetch('/api/admin/campaign/audience', { headers: getAuthHeaders() });
      if (res.ok) {
        const data = await res.json();
        setAudienceReport({
  ...
  const handleRefreshAudience = async () => {
    ...
    setSuccessMsg('');
    try {
      const res = await fetch('/api/admin/campaign/refresh-audience', {
        method: 'POST',
      ...
      if (res.ok) {
        ...
        setErrorMsg(data.error || 'Failed to refresh audience.');
      }
    } catch (err) {
  ...
  const handleExecuteConfirmedCampaign = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setErrorMsg(data.error || 'Failed to trigger campaign.');
      }
    } catch (err) {
  ...
  const handleSendIndividualPaymentLink = async (email, name, plan = 'Premium', amount = 499) => {
    ...
    setSuccessMsg('');
    try {
      const res = await fetch('/api/admin/send-customer-payment-link', {
        method: 'POST',
        headers: getAuthHeaders(),
      ...
      const data = await res.json();
      if (res.ok) {
        setSuccessMsg(data.message || `Payment link sent to ${email}`);
        await fetchCustomers();
        setTimeout(() => setSuccessMsg(''), 4500);
      } else {
        setErrorMsg(data.error || 'Failed to dispatch payment link.');
      }
    } catch (err) {
      setErrorMsg('Payment link error: ' + err.message);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSendBulkPromoPaymentLinks = async () => {
    ...
    setSuccessMsg('');
    try {
      const res = await fetch('/api/admin/send-bulk-promotional-links', {
        method: 'POST',
        headers: getAuthHeaders(),
      });
      const data = await res.json();
      if (res.ok) {
        ...
        setErrorMsg(data.error || 'Failed to send bulk payment links.');
      }
    } catch (err) {
      setErrorMsg('Bulk links error: ' + err.message);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSendAdminOtp = async (e) => {
    ...
    try {
      ...
      setSuccessMsg('Security code sent to authorized Gmail.');
      setResendTimer(30);
      setView('otp');
    } catch (err) {
