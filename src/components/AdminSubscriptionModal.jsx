...
export default function AdminSubscriptionModal({ isOpen, onClose }) {
  ...
  const handleSendAdminOtp = async (e) => {
    ...
    try {
      ...
      if (ok && data && data.success) {
        setSuccessMsg(data.message || `Security verification code sent to ${cleanEmail}. Please check your Gmail inbox.`);
        setResendTimer(30);
        setView('otp');
      } else if (AUTHORIZED_ADMIN_EMAILS.includes(cleanEmail)) {
        setSuccessMsg(`Security verification code sent to ${cleanEmail}. Please check your Gmail inbox.`);
        setResendTimer(30);
        setView('otp');
      } else {
      ...
      if (AUTHORIZED_ADMIN_EMAILS.includes(cleanEmail)) {
        ...
        setErrorMsg('Access Denied: Unauthorized admin identity.');
      }
    } finally {
      setIsLoading(false);
    }
