...
export function StatTile({ label, value, sub, accent, badge, bgImage, className = '', isLocked = false, onUnlock }) {
  ...
  return (
    <div
      ...
      className={`relative isolate overflow-hidden rounded-[22px] border border-[#E7ECF4] bg-white p-6 shadow-sm-clean transition-all duration-350 ${isLocked ? 'cursor-pointer hover:border-[#161C2D]' : 'hover:-translate-y-1 hover:shadow-card'} group ${className}`}
    >
      {bgImage && (
        <>
          <img
      ...
      <div className={`relative z-10 transition-all duration-500 ${isLocked ? 'blur-[6px] opacity-60 pointer-events-none select-none group-hover:blur-[8px]' : ''}`}>
...
export function EditableStatTile({ label, valueNumber, prefix = '', sub, badge, bgImage, onSave, accent = '', className = '' }) {
  ...
  return (
    <div
      className={`relative isolate overflow-hidden rounded-[22px] border border-[#E7ECF4] bg-white p-6 shadow-sm-clean transition-all duration-350 group ${className}`}
    >
      {bgImage && (
        <>
          <img
