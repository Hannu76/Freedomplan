...
import { AnimatedCounter, BANKNOTE_URL, BEFORE_URL, BLACK_TEXT_URL } from './ui.jsx';
  ...
  return (
    <div className="w-full mt-6 mb-8 animate-slide-up animation-delay-200">
      <div className="flex items-center justify-center gap-2 mb-4 text-center">
        <h3 className="text-base sm:text-lg font-black uppercase tracking-widest text-center flex items-center justify-center gap-x-[0.24em] flex-wrap w-full">
          <span
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {CURRENCIES.map((currency) => {
          const rate = effectiveRates ? (effectiveRates[currency.code] || 1) : (currency.code === 'GBP' ? 1 : 0);
            ...
            >
              ...
              <div className="relative z-20 flex flex-col justify-between h-full ">
                <div className="flex items-center justify-between mb-2">
                  <p className="text-[11px] font-bold uppercase tracking-wider text-[#161C2D] transition-colors">
                    {currency.label} — {currency.code}
                  </p>
                </div>

                <div className="flex items-center gap-1 transition-all duration-200">
                  <p className="figure text-xl sm:text-2xl font-extrabold tracking-tight text-[#161C2D] transition-colors">
                    {effectiveRates && Object.keys(effectiveRates).length > 0 ? (
