...
export default function CurrencyConverter() {
  const { rate, effectiveRates, derived } = useStore()
  const savingsYearlyTarget = derived?.savingsYearlyTargetGBP || (derived?.planMonthlySavingsGBP ? derived.planMonthlySavingsGBP * 12 : 5469)

  const [gbpInput, setGbpInput] = useState(savingsYearlyTarget)
  ...
  // Automatically sync with Savings page yearly target as source of truth
  useEffect(() => {
    if (!isCustomEdited) {
      setGbpInput(savingsYearlyTarget)
    }
  }, [savingsYearlyTarget, isCustomEdited])
  ...
  // Quick preset amounts (Yearly Plan preset dynamically anchored to Savings page target)
  const presets = [
    { label: '£1,000', value: 1000 },
    { label: '£2,500', value: 2500 },
    { label: '£5,000', value: 5000 },
    { label: `£${savingsYearlyTarget.toLocaleString('en-GB')} (Yearly Plan)`, value: savingsYearlyTarget },
            ...
            {presets.map((p) => {
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    setGbpInput(p.value)
                    setIsCustomEdited(p.value !== savingsYearlyTarget)
                  }}
            ...
            <div className="relative flex items-center">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  setGbpInput(val)
                  setIsCustomEdited(val !== savingsYearlyTarget)
                }}
