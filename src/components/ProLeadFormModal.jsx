...
import MarketingConsentCheckbox from './marketing/MarketingConsentCheckbox'

const isLocalhost = typeof window !== 'undefined' && (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1');
const API_BASE_URL = '/api/auth'

export default function ProLeadFormModal({ isOpen, onClose, onUnlock }) {
  const { users, setUsers, setIsLoggedIn, setCurrentUser, setBasicLoan, setSessionLoginTime } = useStore()
  ...
  const isEmailRegistered = (email) => {
    ...
    const cleanEmail = email.toLowerCase().trim()
    const inUsers = users.some(u => u.email?.toLowerCase().trim() === cleanEmail)
    let inRegisteredList = false
    try {
      const existing = JSON.parse(localStorage.getItem('freedomPlan.registeredEmails') || '[]')
      inRegisteredList = existing.some(e => e.toLowerCase().trim() === cleanEmail)
    } catch (e) { }
  ...
  const handleEmailCheck = (e) => {
    ...
    const cleanEmail = checkEmail.toLowerCase().trim()
    if (isEmailRegistered(cleanEmail)) {
      const userExists = findUserByEmail(cleanEmail)
      setFormData(prev => ({ ...prev, email: cleanEmail, name: userExists?.name || prev.name }))
      setEmailCheckMessage('You are an existing customer! We have smoothly directed you to Existing Account Sign In.')
      setView('login-request')
    } else {
      setFormData(prev => ({ ...prev, email: cleanEmail }))
      setEmailCheckMessage('No existing account was found for this email. We have smoothly directed you to Register a new account.')
      setView('register')
    }
  }
  ...
  const handleChange = (e) => {
    ...
    if (name === 'email' && isValidEmail(value)) {
      ...
      if (view === 'register' && isEmailRegistered(cleanEmail)) {
        ...
        setEmailCheckMessage('You are an existing customer! We have smoothly switched you to Existing Account Sign In.')
        setView('login-request')
      }
    }
  }
  ...
  const handleRegisterSubmit = async (e) => {
    ...
    // Smooth check: If user attempts to register with an existing registered email
    if (isEmailRegistered(cleanEmail)) {
      const existingUser = findUserByEmail(cleanEmail)
      setFormData(prev => ({ ...prev, name: existingUser?.name || prev.name, email: cleanEmail }))
      setEmailCheckMessage('You are an existing customer! We have smoothly switched you to Existing Account Sign In so you can request an OTP.')
      setView('login-request')
      return
    }
    ...
    setTimeout(() => {
      ...
      setIsLoggedIn(true)
      setSessionLoginTime(Date.now()) // Stamp session start for 5-min rule
  ...
  const completeProLeadVerification = () => {
    // Login Successful
    const timestamp = new Date().toISOString()
    const normalizedEmail = formData.email.toLowerCase().trim()
    saveRegisteredEmail(normalizedEmail)

    let loggedInUser = users.find(u => u.email?.toLowerCase().trim() === normalizedEmail)
    ...
    if (!loggedInUser) {
      ...
      updatedUsers = users.map(u =>
        u.email?.toLowerCase().trim() === normalizedEmail ? { ...u, lastLogin: timestamp } : u
      )
    }
            ...
            <form onSubmit={(e) => e.preventDefault()} className="flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-2 p-3 bg-slate-50 border border-slate-200 rounded-xl">
                ...
                <div className="flex items-center justify-center gap-4">
                  <span className={`text-xs font-black transition-colors ${isOutsideUK === false ? 'text-emerald-600 font-extrabold scale-105' : 'text-neutral-400'}`}>
                    Right now in UK
                  </span>
                  <div className="bauble_box">
                    <input className={`bauble_input ${isOutsideUK === null ? 'unselected' : ''}`} id="bauble_check" name="bauble" type="checkbox" checked={isOutsideUK === true} onChange={(e) => { setIsOutsideUK(e.target.checked); setError(''); }} />
              ...
              <div className="pt-1 pb-1">
                <MarketingConsentCheckbox
                  ...
                  label="I want to receive weekly promotional offers & creator earnings opportunities (every Friday)."
                />
              </div>
        ...
        {view === 'login-request' && (
          <div className="animate-fade-in relative z-10">
            ...
            <div className="mt-8 border-t border-[#EEF2F7] pt-6">
              ...
              <button
                onClick={handleShare}
                className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-[#001C44] text-white rounded-xl text-xs font-extrabold uppercase tracking-wide shadow-md hover:opacity-90 transition-colors"
              >
