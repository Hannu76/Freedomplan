...
export default function ProLeadFormModal({ isOpen, onClose, onUnlock }) {
  const { users, setUsers, setIsLoggedIn, setCurrentUser, setBasicLoan, setSessionLoginTime, currentUser, isLoggedIn } = useStore()

  // Views: 'form' (main vertical registration) | 'login' (existing user email) | 'existing-otp' (OTP verification) | 'registration-success'
  ...
  // ─────────────────────────────────────────────────────────────
  // 2. VERTICAL FORM SUBMIT (NEW USER REGISTRATION - NO OTP)
  // ─────────────────────────────────────────────────────────────
  const handleVerticalFormSubmit = async (e) => {
    ...
    if (isOutsideUK === false) {
      ...
      if (!clean.startsWith('+')) {
        setError('Please enter your phone number with your country code.')
        return
      }
      const digits = clean.slice(1).replace(/\D/g, '')
      if (digits.length < 7 || digits.length > 15) {
        setError('Please enter a valid international phone number with country code (e.g. +919876543210).')
        return
      }
  ...
  // ─────────────────────────────────────────────────────────────
  // 3. QUICK LOGIN SUBMIT (EXISTING CUSTOMER ONLY - DISPATCHES OTP)
  // ─────────────────────────────────────────────────────────────
