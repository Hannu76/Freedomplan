...
const API_BASE_URL = 'https://freedomplan-backend.onrender.com/api/auth'

export default function ProLeadFormModal({ isOpen, onClose, onUnlock }) {
  const { users, setUsers, setIsLoggedIn, setCurrentUser } = useStore()
  
  // view can be 'register', 'login-request', 'login-otp'
  ...
  const handleRegisterSubmit = async (e) => {
    ...
    if (isDuplicate) {
      setError('An account with these details already exists. Please log in to your existing account.')
      return
    }
  ...
  const handleLoginRequest = async (e) => {
    ...
    setIsSubmitting(true)
    try {
      if (API_BASE_URL.includes('api.freedomplan.com') || isLocalhost) {
        // Mock OTP send for frontend-only deployment
  ...
  const verifyOtp = async () => {
    ...
    try {
      if (API_BASE_URL.includes('api.freedomplan.com') || isLocalhost) {
        // Mock OTP verification for frontend-only deployment
        await new Promise(resolve => setTimeout(resolve, 800));
        if (code !== '123456') {
          throw new Error('Invalid OTP. Please use 123456 to login.');
        }
      } else {
      ...
      setCurrentUser(updatedUsers.find(u => u.email.toLowerCase() === normalizedEmail))
      setIsLoggedIn(true)
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          ...
          url: 'https://freedomplan.com' // Placeholder to be replaced when live
        })
      } catch (error) {
          ...
          <div className="animate-fade-in relative z-10">
            ...
            <form onSubmit={(e) => e.preventDefault()} className="flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-4 mt-2">
                <button type="button" onClick={handleRegisterSubmit} disabled={isSubmitting} className="w-full px-4 py-3 rounded-xl text-[13px] font-extrabold tracking-wide uppercase text-white transition-all duration-200 active:scale-95 disabled:opacity-50 text-center" style={{ background: 'linear-gradient(135deg, #0034de 0%, #006eff 100%)', boxShadow: '0px 2px 4px rgba(0,13,56,0.28), inset 0px 4px 5px #0070f0, inset 0px -4px 5px #002cbb' }}>
                  {isSubmitting ? 'Wait...' : 'Register Basic Plan'}
                </button>
              </div>
              
              <button type="button" onClick={() => { setView('login-request'); setError(''); }} className="mt-2 text-sm font-bold text-neutral-500 hover:text-neutral-900 transition-colors">
