...
export default function Dashboard({ onViewReport, onRequirePro }) {
  const { derived, rate, customPlan, updateCustomPlan, basicLoan, setBasicLoan, darkMode, setDarkMode, isProUnlocked, isLoggedIn, setIsLoggedIn, sessionDownloadCount, setSessionDownloadCount } = useStore()
  const { 
    savedThisMonth, 
    savedThisYear, 
    savedAllTime, 
...
If you need assistance using the platform or understanding your report, our support team is available through the website.`;
  ...
  const handleDownloadStrategy = () => {
    ...
    if (sessionDownloadCount >= 3) {
      alert("You have reached the maximum of 3 downloads allowed per 5-minute session. Please sign in again.")
      setIsLoggedIn(false)
      if (onRequirePro) onRequirePro('dashboard')
      return
    }
    setSessionDownloadCount(prev => prev + 1)
    performDownload()
