...
export default function Dashboard({ onViewReport, onRequirePro }) {
  const { derived, rate, customPlan, updateCustomPlan, basicLoan, setBasicLoan, darkMode, setDarkMode, isProUnlocked } = useStore()
  const {
    savedThisMonth,
    savedThisYear,
    savedAllTime,
  ...
  const performDownload = () => {
    ...
    const monthlyExpenses = `£${totalExpenses.toLocaleString('en-GB')}`;
    const monthlyTarget = `£${targetMonthlySavingsGBP.toLocaleString('en-GB')}`;
    const monthlyBuffer = `£${moneyLeftAfterSavings.toLocaleString('en-GB')}`;
    const monthlyPayment = `₹${(basicLoan * 0.0125).toLocaleString('en-IN')}`;
    const exchangeRate = `£1 = ₹${rate.toFixed(2)}`;

    // Calculate progress based on what has been saved vs total estimated debt (using rate as a proxy)
    const estimatedTotalDebtGBP = (basicLoan / rate) + savedAllTime;
    const progressPercentNum = estimatedTotalDebtGBP > 0 ? (savedAllTime / estimatedTotalDebtGBP) * 100 : 0;
    const progressPercent = progressPercentNum.toFixed(1);

    const completionDate = new Date(new Date().setMonth(new Date().getMonth() + remainingMonths)).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
    const repaymentStatus = strategyChecks.every(Boolean) ? 'On Track' : 'Needs Attention';

    const aiSummary = `Based on your current trajectory, you are dedicating ${monthlyTarget} monthly to clear your loan. With a buffer of ${monthlyBuffer}, you have ${moneyLeftAfterSavings >= 0 ? 'room to potentially accelerate your payments.' : 'limited flexibility and should monitor expenses closely to avoid a deficit.'}`;

    const blocks = Math.round((progressPercentNum / 100) * 16);
    const progressBarCurrent = '█'.repeat(blocks) + '░'.repeat(16 - blocks);
    const progressBarMid = '█'.repeat(8) + '░'.repeat(8);

    const reportText = `# PERSONALIZED DEBT FREEDOM STRATEGY REPORT

### Prepared exclusively from your financial profile
...
This report provides a personalized loan repayment strategy based on the financial information you provided. It evaluates your current loan balance, income, expenses, savings capacity, and repayment plan to give you a clear overview of your financial position.

Whenever your financial information is updated, this report is automatically regenerated to ensure your repayment strategy and projections remain accurate and aligned with your latest financial profile.
...
| Monthly Living Expenses  |        ${monthlyExpenses} |
| Monthly Target Savings   |          ${monthlyTarget} |
| Available Monthly Buffer |          ${monthlyBuffer} |
| Monthly Loan Repayment   |         ${monthlyPayment} |
...
Repayment progress is automatically updated based on your latest financial information and repayment activity.
...
Based on your current financial profile, your repayment plan is evaluated using your income, monthly expenses, savings capacity, loan balance, repayment amount, remaining loan term, and exchange rate (where applicable).

The strategy is designed to help you maintain sustainable repayments while preserving a healthy monthly cash buffer. Whenever your financial information changes, the analysis is automatically updated to reflect your latest financial situation.

---

# Key Insights

• Your repayment strategy is based on your latest financial information.

• Maintaining consistent repayments helps keep your projected loan completion on schedule.

• Your available monthly buffer provides additional flexibility for unexpected expenses.

• Regularly updating your financial information ensures future repayment projections remain accurate.

• Any changes to your income, expenses, savings, or exchange rate will automatically be reflected in your next generated report.

---

# ⚠️ Important Information

> **This report is generated automatically using the financial information you provide. It is intended to support budgeting, repayment planning, and financial organization. It should not be interpreted as regulated financial, legal, tax, or investment advice. Actual repayment outcomes depend on your lender's terms, applicable interest rates, exchange rate movements, repayment behavior, and future financial circumstances.**

---

# Need Assistance?

Your repayment strategy is automatically recalculated whenever you update your financial information through your dashboard.

If your income, expenses, savings, loan details, or exchange rate change, simply generate a new report to receive an updated repayment strategy based on your latest financial profile.

If you need assistance using the platform or understanding your report, our support team is available through the website.`;

    const blob = new Blob([reportText], { type: 'text/markdown;charset=utf-8' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = `Repayment_Strategy_Report_${reportId}.md`;
    link.click();
  ...
  const handleDownloadStrategy = () => {
    if (!isProUnlocked) {
      setWantsToDownload(true)
      if (onRequirePro) onRequirePro('dashboard')
      return
    }
    performDownload()
...
action = {
  ...
  <><div className="overflow-hidden w-full mb-8 rounded-[22px]">
    ...
    className="relative isolate overflow-hidden rounded-[20px] bg-neutral-950 border border-neutral-800 shadow-xl min-h-[160px] group transition-all duration-350 hover:-translate-y-0.5 hover:shadow-2xl cursor-pointer"
  ...
  >
      ...
      >
        ...
        <div className="absolute inset-0 w-full h-full p-6 flex flex-col items-center justify-center gap-1" style={{ backfaceVisibility: 'hidden' }}>
          <img
            src="/images/saveing.png"
            alt="Net Monthly Surplus Buffer"
            className="absolute inset-0 w-full h-full object-cover object-[center_25%] z-[-2] transition-transform duration-700 group-hover:scale-105 opacity-100" />

          <span className="text-xs font-bold text-white uppercase tracking-wider mb-2 relative z-10 drop-shadow-md">Net Monthly Surplus Buffer</span>
          <div className="relative z-10 flex items-center justify-center gap-3">
            <span className={`figure text-4xl sm:text-5xl font-extrabold ${moneyLeftAfterSavings >= 0 ? 'text-white' : 'text-rose-400'}`}>
              £{moneyLeftAfterSavings.toFixed(0)}
            </span>
            <span className="px-3.5 py-1.5 rounded-full bg-neutral-900/90 text-white text-xs font-bold border border-neutral-700 shadow-sm mt-2">
              / month
            </span>
          </div>
        </div>
  ...
  < div className = "flex justify-center gap-6 w-full mt-6 flex-wrap items-stretch" >
    ...
    < div className = "luminous-card-container shrink-0" >
      <><input type="checkbox" className="luminous-toggle-input" id="lum-toggle" /><div className="luminous-card">
        ...
        <div className="luminous-content">
          <div className="luminous-icon flex items-end justify-center gap-1.5 pb-2">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 text-yellow-500 opacity-60"><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z" /></svg>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-7 h-7 text-yellow-400 opacity-80"><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z" /></svg>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-12 h-12 text-yellow-300 drop-shadow-[0_0_12px_rgba(253,224,71,0.8)]"><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z" /></svg>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-7 h-7 text-yellow-400 opacity-80"><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z" /></svg>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 text-yellow-500 opacity-60"><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z" /></svg>
          </div>
    ...
    <div className="relative flex flex-col gap-5">
      ...
      <div className="flex divide-x divide-neutral-800">
        ...
        <div className="flex-1 pl-6">
          ...
          <p className={`mt-1 text-xs font-medium ${totalExpenses + customPlan.monthlySavingsTarget > customPlan.monthlyIncome ? 'text-rose-400' : 'text-neutral-400'}`}>
...
{/* Custom Target Edit Warning */ }
{
  customTargetDeficit > 0 && !isCustomTargetNotificationDismissed && (
    <div className="fixed bottom-4 left-4 right-4 sm:left-1/2 sm:right-auto sm:-translate-x-1/2 sm:w-full max-w-lg z-50 animate-slide-up">
      <div className="bg-neutral-950/95 backdrop-blur-xl border border-neutral-800/50 rounded-2xl shadow-[0_10px_40px_rgba(0,0,0,0.5)] overflow-hidden transition-all duration-300">
