...
export default function AnalyticsDashboard({ onRequirePro }) {
  const { timeline, entries, rate, setRate, derived, customPlan, basicLoan, setBasicLoan, isProUnlocked } = useStore()
  ...
  const [interestRate, setInterestRate] = useState(9) // Default to 9% like screenshot
  const [simulatorYears, setSimulatorYears] = useState(10) // Default to 10 years like standard repayment
  const [moratoriumMonths, setMoratoriumMonths] = useState(36)
  const [coApplicantContribution, setCoApplicantContribution] = useState(5000)
          ...
          <div className="flex flex-col gap-5">
            <div className="bg-[#F9FBFD] p-5 rounded-[16px] border border-[#EEF2F7]">
              <div className="flex justify-between items-center mb-5">
                ...
                <span className="text-3xl font-bold text-[#161C2D] figure tracking-tight">₹{rate} <span className="text-sm text-[#667085] font-semibold">/ £</span></span>
              </div>
              <input 
                type="range" 
                min="115" 
            ...
            {/* Impact Calculation */}
            {(() => {
              const inrVal = PLAN.yearlyTarget * rate;
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 mt-4 w-full">
              ...
              <button 
                onClick={() => alert("Flywire Form will be implemented here.")}
                className="flex flex-1 items-center justify-center gap-2 w-full py-3 px-4 rounded-[16px] border border-[#EEF2F7] bg-white shadow-sm hover:shadow-md hover:border-[#D0D5DD] transition-all group overflow-hidden"
                title="Flywire"
              >
                <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/1/1e/Flywire_logo.svg/512px-Flywire_logo.svg.png" alt="Flywire" className="h-6 object-contain opacity-80 group-hover:opacity-100 transition-opacity" />
                <span className="text-xs font-bold text-[#161C2D]">Flywire</span>
              </button>
            </div>
          </div>
        </Card>
          ...
          <div className="mt-8 mb-10 animate-slide-in-bottom delay-300">
            <div className="flex flex-col lg:flex-row gap-6">
              ...
              <div className="flex-1 bg-white border border-[#EEF2F7] rounded-[24px] shadow-sm-clean p-6 lg:p-8 flex flex-col justify-start space-y-8">
                ...
                {/* Premium Image Header - 3D Overlap Effect (Requires Transparent PNG) */}
                <div className="w-full relative group -mt-10 -mb-12 z-20 pointer-events-none">
                  <img 
                    src="/images/freedomplan.png" 
                    alt="Freedom Plan" 
                    className="w-full h-auto max-h-[340px] object-contain object-bottom drop-shadow-[0_20px_30px_rgba(0,0,0,0.3)] transition-transform duration-700 group-hover:scale-105 group-hover:-translate-y-2"
                  />
                </div>
              ...
              <div className="flex-1 flex flex-col gap-6">
                ...
                <div className="bg-gradient-to-br from-lime-400 to-lime-500 rounded-[24px] p-8 text-center shadow-xl shadow-lime-500/20 text-neutral-950 relative overflow-hidden group border border-lime-300">
                  ...
                  <h3 className="text-xs font-bold uppercase tracking-widest text-lime-900/70 mb-4 relative z-10">Your Yearly Interest Burden</h3>
                  <div className="flex items-center justify-center gap-2 mb-3 relative z-10">
                    <span className="text-6xl font-black tracking-tighter figure leading-none">
                       <AnimatedCounter prefix="₹" value={Math.round(yearlyInterest)} />
                    </span>
                  </div>
                  <p className="text-sm font-semibold text-lime-900/80 mb-8 relative z-10">
                    for {M} months study period at {interestRate}% interest
                  </p>
