...
export default function AnalyticsDashboard({ onRequirePro, onRequireLogin }) {
  ...
  const currentIncome = customPlan.monthlyIncome || PLAN.monthlyIncome
  ...
  const cashFlowSlices = [
    { label: 'Rent', amount: rentAvg, color: 'bg-rose-500', stroke: '#f43f5e', pct: (rentAvg / currentIncome) * 100 },
    { label: 'Bills & Utilities', amount: billsTravel, color: 'bg-amber-500', stroke: '#f59e0b', pct: (billsTravel / currentIncome) * 100 },
    { label: 'Target Savings', amount: savingsTarget, color: 'bg-[#B6F36A]', stroke: '#B6F36A', pct: (savingsTarget / currentIncome) * 100 },
    { label: 'Buffer & Discretionary', amount: surplus, color: 'bg-[#4A7BFF]', stroke: '#4A7BFF', pct: (surplus / currentIncome) * 100 },
  ]
  ...
  return (
    <div className="space-y-6 animate-slide-up">
      ...
      <div className="flex justify-center w-full">
        <div className="group relative w-80 max-w-[320px] overflow-hidden rounded-2xl bg-neutral-950 p-6 font-sans shadow-2xl border border-neutral-800 double-invert">
          ...
          <div className="relative flex flex-col gap-5">
            ...
            <div className="relative h-28 w-full flex flex-col justify-between pt-1 group/chart">
              ...
              {/* 3-Year Repayment Plan Category Bar Chart with Masked *** Data Labels */}
              {(() => {
                const scheduleData = calculateDynamicSchedule({ basicLoan, interestRate, rate, moratoriumMonths, derived, customPlan });
                const categories = [
                  { name: 'Yr 1', val: scheduleData.yrSchedule[0]?.principalRepaid || 700000 },
                  { name: 'Yr 2', val: scheduleData.yrSchedule[1]?.principalRepaid || 800000 },
                  { name: 'Yr 3', val: scheduleData.yrSchedule[2]?.principalRepaid || 900000 },
                  { name: 'EMI', val: scheduleData.annualEMI || 350000 },
                  { name: 'Prepay', val: scheduleData.yearlyPrepayment || 450000 },
                  { name: 'Saved', val: scheduleData.totalInterestSaved || 600000 },
                  { name: 'Buffer', val: (customPlan.monthlyIncome || 1350) * 12 },
                  { name: 'Target', val: scheduleData.loanAmount || 2500000 },
                ];
                ...
                return (
                  <div className="relative z-10 flex items-end justify-between gap-1 sm:gap-1.5 h-full w-full px-0.5">
                    {categories.map((cat, idx) => {
                      ...
                      return (
                        <div key={idx} className="flex-1 flex flex-col items-center h-full justify-end group/bar relative cursor-pointer" title="Unlock Pro Access to view exact details">
                          {/* Tooltip on Hover with Masked Data (Pure White Text) */}
                          ...
                          {/* Data label masked with *** above bar (Pure White) */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
        ...
        <Card eyebrow="Monthly Budget" title="Income Allocation Gauge">
          ...
          {(() => {
            const activeIndex = hoveredSlice !== null ? hoveredSlice : 0;
            const activeSlice = cashFlowSlices[activeIndex];
            ...
            return (
              <>
                <div
                  className="relative w-full h-80 mx-auto mb-2 flex items-center justify-center group cursor-pointer"
                  onMouseLeave={() => setHoveredSlice(null)}
                >
                  <svg
                    viewBox="0 0 42 42"
                    className="w-72 h-72 overflow-visible relative z-10"
                  >
                    {(() => {
                        ...
                        return (
                          <g key={i}>
                            ...
                            <circle
                              ...
                              style={{
                                transform: 'scale(1)',
                                filter: isHovered ? `drop-shadow(0px 0px 8px ${slice.stroke})` : 'none'
                              }}
                            />
                          </g>
                        )
                  ...
                  {/* Center Constant Details (Always active) */}
                  <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none transition-all duration-300 z-10">
                    <div className="flex flex-col items-center mt-2 animate-fade-in text-center">
                      <span className="text-[11px] font-bold uppercase tracking-wider text-[#667085] mb-1">{activeSlice.label}</span>
                      <span className="text-3xl font-bold text-[#161C2D] figure leading-none">£{amount}</span>
                      <div className="flex items-center gap-2 mt-2 bg-white/95 backdrop-blur-md px-3 py-1 rounded-full shadow-sm border border-[#EEF2F7]">
                        <span className="h-2 w-2 rounded-full" style={{ backgroundColor: activeSlice.stroke }} />
                        <span className="text-[11px] font-bold" style={{ color: activeSlice.stroke }}>{pct}% Utilisation</span>
                      </div>
                    </div>
                  </div>
                </div>

                {/* Contextual Analysis Panel (Always Active) */}
                <div className="mt-3 p-4 rounded-2xl bg-white border border-[#EEF2F7] shadow-sm transition-all duration-300 text-left">
          ...
          <div className="mt-8 mb-10 animate-slide-in-bottom delay-300">
            <div className="flex flex-col lg:flex-row gap-6">
              ...
              <div className="flex-1 bg-white border border-[#EEF2F7] rounded-[24px] shadow-sm-clean p-6 lg:p-8 flex flex-col justify-start space-y-8 relative">
                ...
                <div className="mt-8">
                  <WalletCard isBasicUnlocked={isBasicUnlocked} onRequirePro={onRequirePro} />
                </div>
              ...
              <div className="flex-1 flex flex-col gap-6">
                ...
                >
                  ...
                  <div
                    ...
                    style={{ background: 'radial-gradient(125% 125% at 50% 10%, #fff 40%, #63e 100%)' }}
                  />
                  ...
                  <div className="w-full flex items-center justify-between pt-2 px-2 relative z-10 border-t border-black/10 mt-auto">
                    ...
                    <a
                      href="https://wa.me/917993144249?text=Hi%20Renuka%2C%20I%20would%20like%20to%20connect%20with%20you%20regarding%20my%20Freedom%20Plan%20queries."
                      target="_blank"
      ...
      <div className="relative z-10 bg-white/80 backdrop-blur-md border border-white/60 rounded-2xl p-3 flex items-center justify-between shadow-md">
        ...
        <div className="relative group shrink-0 ml-2">
          ...
          <button
            type="button"
            onClick={(e) => { e.stopPropagation(); onDownload && onDownload(); }}
            className="relative z-10 py-2 px-4 rounded-full font-extrabold uppercase tracking-wide text-[11px] text-[#052E16] transition-all bg-[#98CD3F] hover:opacity-90 shadow-xl border border-[#7DB425]/50 flex items-center justify-center gap-1.5 overflow-hidden whitespace-nowrap active:scale-95"
            style={{
...
function WalletCard({ isBasicUnlocked, onRequirePro }) {
  return (
    <div className="w-full flex justify-center pt-24 pb-12 mb-6">
      <div className={`wallet ${isBasicUnlocked ? 'pro-unlocked' : ''}`}>
