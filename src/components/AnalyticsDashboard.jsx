...
export default function AnalyticsDashboard({ onRequirePro, onRequireLogin, onTriggerWhatsApp }) {
  ...
  const handlePollVote = async () => {
    setIsVoting(true);
    try {
      localStorage.setItem('freedomplan_poll_voted', 'true');
    } catch (error) {
      console.warn('Poll vote recording warning:', error);
    }
    setTimeout(() => {
      setIsVoting(false);
      setHasVoted(true);
...
`;
  ...
  // ── Redirect / Navigate to Top 3-Year Download Section from Flipping Card ──
  const handleFlipCardDownload = (e) => {
    if (e && e.stopPropagation) e.stopPropagation();
    const target = document.getElementById('top-3year-plan-download');
    if (target) {
      target.scrollIntoView({ behavior: 'smooth', block: 'center' });
      target.classList.add('ring-4', 'ring-[#00cc44]/70', 'transition-all', 'duration-500');
      setTimeout(() => {
        target.classList.remove('ring-4', 'ring-[#00cc44]/70');
      }, 2500);
    }
    handleDownloadYearlyReport();
  };
  ...
  return (
    <div className="space-y-6 animate-slide-up">
      ...
      <div className="flex justify-center w-full">
        <div className="group relative w-80 max-w-[320px] overflow-hidden rounded-3xl bg-neutral-950 p-6 font-sans shadow-2xl border border-neutral-800 double-invert">
          ...
          <div className="relative flex flex-col gap-5">
                ...
                <>
                  <div className="flex divide-x divide-neutral-800">
                    <div className="flex-1 pr-4">
                      ...
                      <p className="mt-1 text-xs font-bold text-[#00cc44]">100%</p>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2.5 text-[9px] font-bold text-neutral-400">
                        <div className="flex items-center gap-1">
                          <span className="w-2 h-2 rounded-full bg-[#00cc44] shadow-sm" />
                          <span>Principal</span>
                        </div>
                        <div className="flex items-center gap-1">
                          ...
                          return (
                            <div key={yr.year} className="flex-1 flex flex-col items-center h-full justify-end group/bar relative">
                              ...
                              <div className="flex items-end justify-center gap-1.5 w-full h-full pb-1">
                                ...
                                <div
                                  className="w-3.5 rounded-t-[4px] bg-gradient-to-t from-[#00cc44]/20 via-[#00cc44] to-[#33d666] transition-all duration-500 group-hover/bar:brightness-125 group-hover/bar:shadow-[0_0_10px_rgba(0,204,68,0.5)] origin-bottom"
            ...
            <div id="top-3year-plan-download" className="border-t border-neutral-800/80 pt-4 flex justify-center scroll-mt-28">
      ...
      {(() => {
        ...
        const effectiveLiveEMI = coApplicant > 0 ? coApplicant : (LOAN.monthlyEMIINR || 5000);
        const fixedEMIAllYears = effectiveLiveEMI * termMonths;
        ...
        const getSliderStyle = (val, min, max) => {
          ...
          return {
            background: `linear-gradient(to right, #00cc44 0%, #00cc44 ${percentage}%, #EEF2F7 ${percentage}%, #EEF2F7 100%)`
          };
        };
            ...
            <div className="flex flex-col lg:flex-row gap-6">
                    ...
                    <div className="mt-auto">
                      {hasVoted ? (
                        ...
                        <button
                          ...
                          disabled={isVoting}
                          className="inline-flex items-center justify-center gap-2 px-6 py-2.5 bg-lime-400 hover:bg-lime-300 text-lime-950 font-bold text-sm rounded-xl transition-colors group/btn shadow-[0_0_20px_rgba(163,230,53,0.15)] w-max disabled:opacity-50"
                        >
                          {isVoting ? (
                            <>
              ...
              <div className="flex-1 flex flex-col gap-6">
                ...
                {(() => {
                  ...
                  const effectiveLiveEMI = coApplicant > 0 ? coApplicant : (LOAN.monthlyEMIINR || 5000);
                  const annualEMI = effectiveLiveEMI * 12;
                  const schedule = [];
                  const baseline = [];
                  let bal = P_grad;
                  let balBase = P_grad;
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        {/* ── FRONT ── */}
                        <GreenCardFront schedule={schedule} R_annual={R_annual} M={M} P_0={P_0} P_grad={P_grad} unpaidInterest={Math.round(studentShare * M)} onDownload={handleFlipCardDownload} />

                        {/* ── BACK ── */}
                        <GreenCardBack schedule={schedule} onDownload={handleFlipCardDownload} />
                      </div>
                    </div>
                  );
                })()}
                  ...
                  <div className="flex flex-col gap-4 mb-8">
                    ...
                    <div className="flex justify-center -my-2 z-10">
                      <div className="bg-white border border-[#EEF2F7] rounded-full p-1.5 shadow-sm">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-[#00cc44]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M19 14l-7 7m0 0l-7-7m7 7V3" /></svg>
                      </div>
                    </div>
                    ...
                    <div className="flex justify-between items-center bg-[#161C2D] border border-[#161C2D] rounded-[14px] p-5 shadow-lg">
                      ...
                      <div className="text-right">
                        <span className="block text-2xl font-black text-[#00cc44] figure">£{liveTargetMonthlySavingsGBP.toLocaleString('en-GB')} / mo</span>
                    ...
                    <div className="bg-[#00cc44]/10 border border-[#00cc44]/30 rounded-[12px] p-3 mt-1">
...
function GreenCardFront({ schedule, R_annual, M, P_0, P_grad, unpaidInterest, onDownload }) {
  ...
  return (
    <div
      className="bg-gradient-to-br from-[#00cc44] to-[#00a838] rounded-[28px] p-6 sm:p-7 text-center shadow-2xl shadow-[#00cc44]/25 text-neutral-950 relative overflow-hidden border border-[#33d666] w-full flex flex-col justify-between gap-4"
    ...
    >
      ...
      <div className="relative z-10 flex flex-col sm:flex-row items-center justify-between gap-3 w-full border-b border-black/15 pb-3">
        <div className="text-center sm:text-left">
          <p className="text-[10.5px] font-extrabold uppercase tracking-[0.18em] text-neutral-950/75">Total Interest Accumulation</p>
          <p className="text-[11px] font-bold text-neutral-950/85">
      ...
      <div className="relative z-10 grid grid-cols-3 gap-3 my-1">
        {[
          ...
          <div key={label} className="bg-white/30 border border-white/40 rounded-2xl px-3 py-3 backdrop-blur-md flex flex-col items-center gap-1.5 relative shadow-xs">
            <div className="flex items-center gap-1">
              <span className="text-[9.5px] font-extrabold uppercase tracking-widest text-neutral-950/75">{label}</span>
      ...
      {/* Footer */}
      <p className="relative z-10 text-[11.5px] font-semibold text-neutral-950/85 leading-relaxed mx-auto max-w-sm">
...
function GreenCardBack({ schedule, onDownload }) {
  return (
    <div
      className="absolute inset-0 bg-gradient-to-br from-[#00cc44] to-[#00a838] rounded-[28px] shadow-2xl shadow-[#00cc44]/25 border border-[#33d666] w-full p-6 sm:p-7 flex flex-col justify-between overflow-y-auto text-neutral-950"
    ...
    >
      ...
      <div className="relative z-10 bg-white/85 backdrop-blur-md border border-white/60 rounded-2xl p-3.5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-md">
        <div>
          <p className="text-xs font-extrabold text-neutral-950 leading-tight">3-Year Repayment Strategy Report</p>
          <p className="text-[10px] text-neutral-900/80 font-medium mt-0.5">Complete yearly schedules, interest breakdown & principal reduction</p>
        </div>
        <div className="relative group shrink-0">
          <div className="absolute -bottom-1.5 left-1/2 -translate-x-1/2 w-4/5 h-3 bg-gradient-to-r from-emerald-700 via-[#00cc44] to-emerald-600 blur-md rounded-full opacity-80 group-hover:opacity-100 group-hover:blur-lg transition-all duration-300 pointer-events-none" />
          <button
            type="button"
            onClick={(e) => { e.stopPropagation(); onDownload && onDownload(); }}
            className="relative z-10 py-2.5 px-4 rounded-full font-extrabold uppercase tracking-wide text-[11px] text-[#052E16] transition-all bg-[#00cc44] hover:opacity-90 shadow-xl border border-emerald-600/50 flex items-center justify-center gap-1.5 overflow-hidden whitespace-nowrap active:scale-95 cursor-pointer"
            style={{
          ...
          >
            ...
            <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-1/2 h-[2px] bg-gradient-to-r from-transparent via-emerald-300 to-transparent rounded-full opacity-90" />
          </button>
        </div>
      </div>
      ...
      <div className="relative z-10 my-2.5 p-3.5 bg-white/25 border border-white/40 rounded-2xl backdrop-blur-md space-y-1.5 shadow-xs">
        <p className="text-xs font-black uppercase tracking-widest text-neutral-950 text-center">3-Year Repayment Strategy</p>
        <p className="text-xs font-semibold leading-relaxed text-neutral-950/90 text-center px-2">
        ...
        <p className="text-[10px] font-medium leading-tight text-neutral-950/80 text-center pt-1 border-t border-black/10">
      ...
      <div className="relative z-10 pt-2 border-t border-black/15 flex items-center justify-between">
        <span className="text-[10px] font-extrabold text-neutral-950/80 uppercase tracking-wider">Analyze Plan with AI:</span>
        <div className="flex items-center gap-4">
          ...
          >
            ...
            <span className="text-[11px] font-extrabold text-neutral-950">Gemini</span>
          </a>
          ...
          >
            ...
            <span className="text-[11px] font-extrabold text-neutral-950">Claude</span>
          </a>
          ...
          >
            ...
            <span className="text-[11px] font-extrabold text-neutral-950">Perplexity</span>
          </a>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="relative inline-flex" onMouseEnter={() => setVisible(true)} onMouseLeave={() => setVisible(false)}>
      <button
        ...
        className="w-4 h-4 rounded-full bg-black/15 border border-black/25 flex items-center justify-center text-[9px] font-black text-neutral-950/70 hover:bg-black/25 transition-colors focus:outline-none"
  ...
  return (
    <div className="animate-confetti-pop bg-gradient-to-br from-lime-50 to-emerald-50 border-2 border-lime-300 rounded-2xl p-6 text-center shadow-lg shadow-lime-100/60 relative overflow-hidden">
      ...
      {['top-2 left-4', 'top-3 right-6', 'top-1 left-1/2', 'top-4 right-12', 'top-2 left-16'].map((pos, i) => (
        <div key={i} className={`absolute ${pos} w-2 h-2 rounded-full animate-pulse`}
