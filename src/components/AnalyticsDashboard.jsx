...
`;
  ...
  return (
    <div className="space-y-6 animate-slide-up">
      ...
      {/* Chart 1: Monthly Balance Dark Card - Matching Screenshot */}
      <div className="flex justify-center w-full">
        <div className="group relative w-80 max-w-[320px] overflow-hidden rounded-3xl bg-neutral-950 p-6 font-sans shadow-2xl border border-neutral-800 double-invert">
          ...
          <div className="relative flex flex-col gap-5">
            ...
            {/* Two Column Metrics */}
            {(() => {
              const income = customPlan.monthlyIncome || PLAN.monthlyIncome || 1300;
              ...
              const balanceBars = [
                { name: 'Rent', pct: Math.round((rent / income) * 100) || 21, isPrimary: true },
                { name: 'Shop', pct: Math.round((shop / income) * 100) || 3 },
                { name: 'Food', pct: Math.round((food / income) * 100) || 5 },
                { name: 'Trans', pct: Math.round((trans / income) * 100) || 7 },
                { name: 'Bills', pct: Math.round((bills / income) * 100) || 8 },
                { name: 'Ent.', pct: Math.round((ent / income) * 100) || 3 },
                { name: 'Health', pct: Math.round((health / income) * 100) || 2 },
                { name: 'Others', pct: Math.round((others / income) * 100) || 2 },
              ];

              const maxPct = Math.max(...balanceBars.map(b => b.pct), 22);
              ...
              return (
                <>
                  ...
                  {/* Category Bar Chart with dashed grid background */}
                  <div className="relative h-28 w-full flex flex-col justify-between pt-2 group/chart">
                    ...
                    <div className="relative z-10 flex items-end justify-between gap-1 sm:gap-1.5 h-full w-full px-0.5">
                      {balanceBars.map((item, idx) => {
                        const barHeightPct = Math.max(16, Math.min(100, Math.round((item.pct / maxPct) * 82)));

                        return (
                          <div key={idx} className="flex-1 flex flex-col items-center h-full justify-end group/bar relative">
                            {/* Data label on top of bar */}
                            <span className="text-[10px] font-extrabold text-white mb-1 tracking-tight">
                              {item.pct}%
                            </span>
                            ...
                            <div
                              key={`${idx}-${barPulse}`}
                              className="w-full max-w-[20px] rounded-t-[5px] bg-gradient-to-t from-lime-500/20 via-lime-400/80 to-lime-300 transition-all duration-300 group-hover/bar:brightness-125 group-hover/bar:shadow-[0_0_10px_rgba(163,230,53,0.5)] origin-bottom"
                              style={{
                                height: `${barHeightPct}%`,
                                animation: `barRiseSlow 1.8s cubic-bezier(0.16, 1, 0.3, 1) ${idx * 70}ms forwards`,
                              }}
                            />

                            {/* Category label below bar */}
                            <span
                              className={`text-[9px] mt-1.5 truncate w-full text-center transition-colors ${
                                item.isPrimary ? 'text-sky-400 font-bold' : 'text-neutral-400 font-semibold'
                              }`}
                            >
                              {item.name}
                            </span>
            ...
            return (
              <>
                ...
                >
                  <svg
                    viewBox="0 0 42 42"
                    className="w-72 h-72 overflow-visible relative z-10 -rotate-90"
                  >
                    {(() => {
                        ...
                        return (
                          <g key={i}>
                            ...
                            <circle
                              ...
                              fill="transparent"
                              stroke={slice.stroke}
                              strokeWidth={isSelectedOrHovered ? "7.5" : "6"}
                              strokeDasharray={dash}
                              strokeDashoffset={offset}
                              opacity={isSelectedOrHovered ? 1 : 0.75}
                ...
                {/* Category Buttons for direct selection */}
                <div className="flex flex-wrap items-center justify-center gap-2 mb-3 pt-2">
                  {cashFlowSlices.map((slice, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className={isSelected ? 'text-slate-300' : 'text-slate-400'}>£{Math.round(slice.amount)}</span>
                      </button>
                    );
                  })}
                </div>
