...
export default function SavingsTracker() {
  ...
  const scheduleData = useMemo(() => {
    ...
    const rows = timeline.map((m, idx) => {
      ...
      // 1. Calculate base target for this month
      ...
      // 2. Apply sequential rollover/surplus
      let rolloverShortfall = 0
      let savingCredit = 0
      let sourceMonthLabel = lastAdjustmentMonthLabel
      let target = monthBaseTarget

      if (isFullNoIncome) {
        // Excluded month has target = 0; previous carryBalance passes through to next active month
        target = 0
        rolloverShortfall = 0
        savingCredit = 0
        sourceMonthLabel = ''
        ...
        if (carryBalance > 0) {
          ...
          target = monthBaseTarget + rolloverShortfall
          ...
          savingCredit = Math.min(monthBaseTarget, availableCredit)
          target = Math.max(0, monthBaseTarget - savingCredit)
  ...
  }, [timeline, entries, derived.planMonthlySavingsGBP, noIncomePeriods])
                    ...
                    >
                      ...
                      <div className="relative z-10 space-y-4">
                        ...
                        <div className="flex items-center justify-between gap-3 border-b border-black/10 pb-3">
                          ...
                          {/* Status Chips */}
                          <div className="flex flex-wrap items-center gap-2">
                            {isNoIncome && m.noIncomeInfo && (
                              <span className="inline-flex items-center gap-1.5 text-[11px] font-extrabold px-3 py-1 rounded-full bg-slate-200 text-[#334155] border border-slate-300 shadow-xs">
                                <RotateCcw className="w-3 h-3 text-[#475569]" />
                                No Income · {m.noIncomeInfo.durationLabel}
                              </span>
                            )}
                            ...
                            {!isNoIncome && m.rolloverShortfall > 0 && (
                            ...
                            {isCompleted && !m.isAhead && m.savingCredit === 0 && !isNoIncome && (
                            ...
                            {isUpcoming && m.savingCredit === 0 && !isNoIncome && (
                        ...
                        {/* Middle Row: Content layout depends on whether month is No Income or Normal */}
                        {isNoIncome ? (
                          /* Standardized No Income State within same Card structure */
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 pt-1">
                            
                            {/* Left: No Income Information */}
                            <div className="rounded-[16px] bg-white/90 backdrop-blur-xs border border-slate-200 p-4 space-y-2 shadow-xs">
                              <div className="flex items-center justify-between">
                                <span className="text-[11px] font-black uppercase tracking-wider text-[#0F172A]">
                                  No Income Period
                                </span>
                                <span className="text-[10px] font-black text-[#475569] bg-slate-100 px-2.5 py-0.5 rounded-md border border-slate-200">
                                  {m.noIncomeInfo?.durationLabel || 'Excluded'}
                                </span>
                              </div>
                              <p className="text-xs font-bold text-[#0F172A]">
                                Reason: <span className="font-medium text-[#475569]">{m.noIncomeInfo?.reason || 'No Income Period'}</span>
                              </p>
                              <p className="text-xs font-bold text-[#0F172A]">
                                Period: <span className="font-medium text-[#475569]">{m.noIncomeInfo?.periodDates || m.label}</span>
                              </p>
                              <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-xs">
                                <span className="font-black text-[#475569] uppercase text-[10px]">Adjusted Target</span>
                                <span className="font-black text-xl text-[#0F172A] figure">£0</span>
                              </div>
                            </div>

                            {/* Right: Actions */}
                            <div className="flex flex-col justify-between space-y-3 p-1">
                              <div className="space-y-1">
                                <span className="text-[11px] font-black uppercase tracking-wider text-[#0F172A] block">
                                  Schedule Status
                                </span>
                                <p className="text-xs font-semibold text-[#64748B]">
                                  This period is excluded from your active earning schedule. Remaining loan balance is recalibrated across active months.
                                </p>
                              </div>
                              ...
                              <div className="flex items-center gap-2.5 pt-2">
                                <button
                                  ...
                                  className="px-4 py-2 rounded-xl text-xs font-black border border-[#CBD5E1] bg-white text-[#0F172A] hover:bg-[#F1F5F9] transition-all shadow-xs cursor-pointer flex items-center gap-1.5 active:scale-95"
                                >
                              ...
                              {m.savingCredit > 0 ? (
                                <div className="space-y-2">
                                  ...
                                  {infoPopoverKey === m.key && (
                                    <div className="p-3 rounded-xl bg-[#0F172A] text-white text-xs shadow-lg animate-fade-in space-y-1.5 border border-slate-700">
                                      ...
                                      <p className="text-slate-300 text-[11px] leading-relaxed">
                                        Your previous £{m.savingCredit.toLocaleString('en-GB')} surplus saving in {m.sourceMonthLabel} has been carried forward to reduce this month's target.
                                      </p>
                                    </div>
                                  )}
                                  ...
                                  <div className="flex items-baseline justify-between pt-0.5">
                                    <div>
                                      ...
                                      <span className="text-[10px] font-semibold text-[#64748B]">Year {planYear.planYearNumber} Target: £{planYear.yearTarget.toLocaleString('en-GB')}</span>
                                    </div>
                                ...
                                <div className="space-y-2">
                                  ...
                                  {infoPopoverKey === m.key && (
                                    <div className="p-3 rounded-xl bg-[#0F172A] text-white text-xs shadow-lg animate-fade-in space-y-1.5 border border-slate-700">
                                      ...
                                      <p className="text-slate-300 text-[11px] leading-relaxed">
                                        {m.sourceMonthLabel}'s shortfall of £{m.rolloverShortfall.toLocaleString('en-GB')} has been carried forward to this month.
                                      </p>
                                    </div>
                                  )}
                                ...
                                <div>
                                  ...
                                  <span className="block text-[10px] font-semibold text-[#64748B]">
                                    Year {planYear.planYearNumber} Target: £{planYear.yearTarget.toLocaleString('en-GB')}
                                  </span>
                                </div>
                              )}
                            </div>
