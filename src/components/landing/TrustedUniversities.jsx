...
function UniversityLogoItem({ university }) {
  return (
    <a
      ...
      className="flex items-center justify-center flex-shrink-0 shrink-0 px-6 sm:px-10 py-1.5 select-none group transition-transform duration-300 hover:scale-105 cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-500/50 rounded-xl"
    >
      <img
        src={university.logo}
        ...
        loading="eager"
        decoding="async"
        className="h-12 sm:h-14 md:h-16 max-h-[64px] max-w-[180px] sm:max-w-[240px] w-auto object-contain opacity-85 group-hover:opacity-100 transition-opacity duration-300 filter dark:brightness-110 pointer-events-none"
      />
    </a>
  );
}
...
export default function TrustedUniversities({ onContact }) {
  // Seamless continuous loop with single duplicated sequence
  const marqueeTrack = [...UNIVERSITIES_DATA, ...UNIVERSITIES_DATA];

  React.useEffect(() => {
    // Preload university logos so assets are primed and never cause layout shifts on mobile
  ...
  return (
    <section
      ...
      className="relative z-30 w-full pt-24 sm:pt-28 md:pt-14 pb-7 sm:pb-8 md:pb-10 px-4 sm:px-6 flex flex-col items-center justify-center bg-[#ffffff] dark:bg-[#0B0F19] border-t border-b border-slate-200/60 dark:border-slate-800/80 transition-colors overflow-hidden"
    >
      ...
      <div className="w-full relative max-w-7xl mx-auto overflow-hidden">
        ...
        {/* Single Continuous Seamless Infinite Marquee Track (One Row Only) */}
        <div className="flex flex-nowrap items-center animate-marquee-infinite py-1">
