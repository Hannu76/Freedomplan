...
function buildLoanSchedule(timeline, basicLoan, targetYearlyLumpSumINR) {
  ...
  return timeline.map((m) => {
    ...
    const displayEmi = Math.round(balance / 100)
    const emiDeduction = Math.min(LOAN.monthlyEMIINR, balance)
    ...
    return { ...m, emi: displayEmi, lumpSum, balanceAfter: balance, isDecember }
  })
}
      ...
      >
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6 p-5 rounded-[16px] bg-[#F9FBFD] border border-[#EEF2F7] text-xs sm:text-sm text-[#667085]">
          ...
          <div className="shrink-0 flex items-center gap-2">
            ...
            <span className="text-[11px] font-extrabold text-black tracking-wider">[DEC] Milestone Month</span>
          </div>
        </div>
              ...
              {schedule.map((s) => (
                ...
                >
                  <td className="py-3.5 px-4 font-bold text-[#161C2D] whitespace-nowrap">
                    {s.label} {s.isDecember && <span className="text-black font-extrabold ml-1.5 text-xs">ANNUAL TRANSFER</span>}
                  </td>
