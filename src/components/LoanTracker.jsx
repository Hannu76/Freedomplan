      ...
      >
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6 p-5 rounded-[16px] bg-[#F9FBFD] border border-[#EEF2F7] text-xs sm:text-sm text-[#667085]">
          ...
          <div className="shrink-0 flex items-center gap-2">
            ...
            <span className="text-[11px] font-extrabold text-red-600 bg-emerald-500/15 px-2.5 py-1 rounded-full border border-emerald-500/30 uppercase tracking-wider">[DEC] Milestone Month</span>
          </div>
        </div>
              ...
              {schedule.map((s) => (
                <tr
                  ...
                  className={`transition-colors hover:bg-[#F9FBFD] ${
                    s.isDecember ? 'bg-emerald-500/10 font-semibold' : ''
                  }`}
                >
                  <td className="py-3.5 px-4 font-bold text-[#161C2D] whitespace-nowrap">
                    {s.label} {s.isDecember && <span className="text-red-600 font-extrabold ml-1.5 text-xs font-mono bg-emerald-500/20 px-2.5 py-0.5 rounded-full border border-emerald-500/30 shadow-sm">ANNUAL TRANSFER</span>}
                  </td>
                  <td className="py-3.5 px-4 figure text-[#667085]">{fmt(s.emi)}</td>
                  <td className="py-3.5 px-4 figure font-bold text-red-600">
