...
export default function PaymentModal({ isOpen, onClose, onSuccess: onSuccessCallback }) {
  ...
  const handleSubmitRequest = async (e) => {
    ...
    try {
      ...
      let res;
      try {
        res = await fetch('/api/request-payment-link', {
          method: 'POST',
        ...
        // Fallback endpoint
        res = await fetch('/api/payment/request-link', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload),
        });
      ...
      if (!res.ok) {
        // If 404, fallback to /api/payment/request-link
        if (res.status === 404) {
          res = await fetch('/api/payment/request-link', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        >
          ...
          >
            ...
            <AnimatePresence mode="wait">
                ...
                >
                  ...
                  <div className="flex flex-col md:w-[350px] shrink-0 p-5 sm:p-7 border-b md:border-b-0 md:border-r border-gray-100 bg-white">
                    ...
                    <div className="mt-3 pt-2 flex items-center justify-center gap-1.5 text-[10px] text-gray-400 font-medium">
                      ...
                      256-bit SSL encrypted · Automatic email delivery
                    </div>
                  </div>
                  ...
                  <div className="flex-1 flex flex-col justify-center p-5 sm:p-7 bg-slate-50/50">
                    <div className="text-left mb-3">
                      ...
                      <p className="text-[11px] text-gray-500">
                        ₹499 / 1 Month · Instant access to all calculators & tools
                      </p>
                    </div>
              ...
              {/* ════════════════════════════════════════════════════════
                  STEP: SUCCESS (Payment link sent)
              ════════════════════════════════════════════════════════ */}
              {step === 'success' && (
                <motion.div
                  key="success-view"
                ...
                >
                  ...
                  <p className="text-sm font-semibold text-gray-800 mb-1">
                    We've sent your FreedomPlan Premium payment link to:
                  </p>

                  <p className="text-sm font-bold text-blue-600 mb-3 bg-blue-50 py-1.5 px-4 rounded-lg border border-blue-100 break-all">
                    {submittedData?.email || currentUser?.email || 'your email'}
                  </p>

                  <p className="text-xs text-gray-600 leading-relaxed mb-4 max-w-md">
                    Please check your <strong>Inbox</strong>, <strong>Promotions</strong>, or <strong>Spam</strong> folder. The email contains your official Razorpay payment button and visual instructions.
                  </p>
                  ...
                  {submittedData && (
                    <div className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3.5 mb-4 text-left text-xs space-y-1.5">
                      <div className="flex justify-between items-center text-gray-600">
                        <span>Recipient:</span>
                        <strong className="text-gray-900 font-semibold">{submittedData.name || 'Valued Customer'}</strong>
                      </div>
                      <div className="flex justify-between items-center text-gray-600">
                        <span>Plan:</span>
                        <strong className="text-blue-700 font-semibold">FreedomPlan Premium (₹499 / 1 Month)</strong>
                      </div>
                      <div className="flex justify-between items-center text-gray-600">
                        <span>Status:</span>
                        <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-800 flex items-center gap-1">
                          <CheckCircle size={10} /> Email Delivered
                        </span>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="w-full bg-blue-50 border border-blue-200/80 rounded-xl p-3 mb-5 text-left flex items-start gap-2.5">
                    ...
                    <p className="text-xs text-blue-900 leading-snug">
                      <strong>Next step:</strong> Open the email, click the payment button to pay ₹499 securely on Razorpay, and reply with your screenshot for instant activation within 1 hour.
                    </p>
                  </div>
                  ...
                  <div className="flex gap-2 w-full">
                    <a
                      ...
                      className="flex-1 py-3 px-4 rounded-xl font-bold text-xs sm:text-sm bg-blue-600 text-white hover:bg-blue-700 transition-all shadow-md text-center flex items-center justify-center gap-1.5 cursor-pointer"
                    >
                      <Mail size={14} /> Open Gmail
                    </a>
                    <button
                      onClick={onClose}
                      className="flex-1 py-3 px-4 rounded-xl font-bold text-xs sm:text-sm bg-gray-900 text-white hover:bg-black transition-all shadow-md active:scale-98 cursor-pointer"
                    >
                      CLOSE
                    </button>
                  </div>
